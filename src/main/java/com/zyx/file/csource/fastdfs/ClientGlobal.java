...
package com.zyx.file.csource.fastdfs;

import com.zyx.file.csource.common.IniFileReader;
import com.zyx.file.csource.common.MyException;
...
public class ClientGlobal {
     ...
     */
    public static void initDefault(String conf_filename) throws FileNotFoundException, IOException, MyException
    {
    ...
    public static void init(String conf_filename) throws FileNotFoundException, IOException, MyException {
        ...
        InputStream infile = ClientGlobal.class.getClass().getResourceAsStream("/" + conf_filename + "");
        props.load(infile);
        String connect_timeout = props.getProperty("connect_timeout");
        String network_timeout = props.getProperty("network_timeout");
        String charset = props.getProperty("charset");
        String http_tracker_http_port = props.getProperty("http_tracker_http_port");
        String http_anti_steal_token = props.getProperty("http_anti_steal_token");
        String http_secret_key = props.getProperty("http_secret_key");
        String tracker_server = props.getProperty("tracker_server");

        g_connect_timeout = (connect_timeout == null ? DEFAULT_CONNECT_TIMEOUT : Integer.valueOf(connect_timeout));
        if (g_connect_timeout < 0) {
            g_connect_timeout = DEFAULT_CONNECT_TIMEOUT;
        }
        g_connect_timeout *= 1000; //millisecond

        g_network_timeout = (network_timeout == null ? DEFAULT_NETWORK_TIMEOUT : Integer.valueOf(network_timeout));
        if (g_network_timeout < 0) {
            g_network_timeout = DEFAULT_NETWORK_TIMEOUT;
        }
        g_network_timeout *= 1000; //millisecond
        ...
        if (tracker_server == null) {
            szTrackerServers = null;
        } else {
            szTrackerServers = new String[]{tracker_server};
        }
        ...
        g_tracker_http_port = (http_tracker_http_port == null ? 80 : Integer.valueOf(http_tracker_http_port));

        g_anti_steal_token = (http_anti_steal_token != null && (http_anti_steal_token.equals("yes") || http_anti_steal_token.equals("true")));

        if (g_anti_steal_token) {
            g_secret_key = http_secret_key;
        }
    }
